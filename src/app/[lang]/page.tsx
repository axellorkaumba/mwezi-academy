...
import { TypewriterText } from "@/components/typewriter-item";
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        <div
          aria-hidden
          className="ember-halo pointer-events-none absolute -top-40 right-[-10%] h-[560px] w-[560px] rounded-full blur-3xl"
        ...
        <div className="mx-auto max-w-6xl px-5 sm:px-8 pt-20 pb-16 sm:pt-28 sm:pb-24">
          ...
          <HeroTitle
            ...
            className="mt-6 max-w-3xl whitespace-pre-line font-display text-5xl leading-[0.98] font-semibold tracking-tight sm:text-6xl lg:text-7xl"
          />
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {dict.categories.items.map((c, i) => (
              <Reveal key={c.name} delay={i * 70}>
                <div className="group rounded-2xl border border-border bg-paper p-6 transition-colors hover:border-ember/60">
                  <span className="index-glow font-mono text-xs text-ink-muted">
      ...
      <section className="border-t border-border">
        <div className="mx-auto max-w-6xl px-5 sm:px-8 py-20">
          ...
          <div className="mt-10 grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
            {dict.why.items.map((w, i) => (
              <Reveal key={w.name} delay={i * 70} className="bg-surface">
                <div className="p-6">
                  <h3 className="font-display text-lg font-semibold">
                    <span className="underline-draw">{w.name}</span>
                  </h3>
          ...
          <ul className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {dict.ai.items.map((item, i) => (
              ...
              >
                ...
                <TypewriterText text={item} startDelay={i * 180} />
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="border-t border-border">
        <div className="mx-auto max-w-6xl px-5 sm:px-8 py-20">
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {dict.audiences.items.map((a, i) => (
              <Reveal key={a.name} delay={i * 70}>
                <SpotlightCard className="flex items-center justify-between gap-4 rounded-2xl border border-border bg-surface p-6">
                  <div>
        ...
        <div className="mx-auto max-w-6xl px-5 sm:px-8 py-20">
          ...
          <div className="marquee-viewport mt-10">
            <div className="marquee-track">
              {[...dict.proof.items, ...dict.proof.items].map((p, i) => (
                <span
                  key={`${p}-${i}`}
                  className="shrink-0 rounded-full border border-border bg-paper px-4 py-2 font-display text-sm font-semibold text-ink-muted"
                >
                  {p}
                </span>
              ))}
      ...
      <section className="relative overflow-hidden border-t border-border">
        <div
          aria-hidden
          className="ember-halo pointer-events-none absolute bottom-[-20%] left-1/2 h-[480px] w-[480px] -translate-x-1/2 rounded-full blur-3xl"
          style={{ background: "radial-gradient(circle, var(--ember), transparent 70%)" }}
        />
        <div className="relative mx-auto max-w-6xl px-5 sm:px-8 py-24 text-center">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href={`/${l}/formations`}
              className="glow-cta inline-flex items-center justify-center rounded-full bg-ink px-6 py-3.5 text-sm font-semibold text-paper transition-colors hover:bg-ember hover:text-accent-ink"
            >
